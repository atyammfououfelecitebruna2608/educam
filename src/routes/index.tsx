  ...
  return (
    <div className="space-y-10">
      <header className="flex flex-col gap-6 sm:flex-row sm:items-end justify-between">
        ...
        <Link to="/verify">
          <Button size="lg" className="shadow-premium animate-in slide-in-from-right-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={<Users className="h-6 w-6 text-primary" />} label="Étudiants Actifs" value={stats.totalStudents} />
        <StatCard icon={<CheckCircle2 className="h-6 w-6 text-success" />} label="Identités Validées" value={stats.verifiedToday} />
        <StatCard icon={<XCircle className="h-6 w-6 text-destructive" />} label="Tentatives Rejetées" value={stats.rejectedToday} />
        <StatCard icon={<Wallet className="h-6 w-6 text-accent" />} label="Volume Financier" value={`${stats.totalAmount.toLocaleString("fr-FR")} XAF`} />
      </div>

      <Card className="p-6">
        <h2 className="font-semibold text-lg mb-4">Vérifications récentes</h2>
        {loading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-muted-foreground border-b">
                  ...
                  <th className="py-2 font-medium">Étudiant</th>
                  <th className="py-2 font-medium">Remettant (reçu)</th>
                  <th className="py-2 font-medium">Montant</th>
                  <th className="py-2 font-medium">Visage</th>
              ...
              <tbody>
                {recent.map((p) => (
                  <tr key={p.id} className="border-b last:border-0">
                    <td className="py-3">{new Date(p.created_at).toLocaleString("fr-FR")}</td>
                    <td className="py-3">
                      {p.students ? (
                        <div>
                          <div className="font-medium">{p.students.full_name}</div>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-xs text-muted-foreground font-medium">{p.students.matricule}</span>
                    ...
                    <td className="py-3">{p.remettant}</td>
                    <td className="py-3">{p.amount ? `${Number(p.amount).toLocaleString("fr-FR")} XAF` : "—"}</td>
                    <td className="py-3">
                      {p.status === "pending" ? (
